...
int ps_listinfo(void) {
    ...
    for (struct proc* proc_ptr = proc; proc_ptr < &proc[NPROC]; ++proc_ptr) {
        ...
        if (proc_ptr->state != UNUSED) {
            ...
            if (proc_ptr->parent) {
                acquire(&(proc_ptr->parent->lock));
                proc_info.parent_id = proc_ptr->parent->pid;
                release(&(proc_ptr->parent->lock));
            } else
                proc_info.parent_id = -1;
            release(&wait_lock);
            if (plist != 0 &&
                copyout(myproc()->pagetable,
                        (uint64) (plist + proc_count * sizeof(proc_info)),
                        (char*) &proc_info,
                        sizeof(proc_info)) < 0) {
                release(&(proc_ptr->lock));
                return -2;
